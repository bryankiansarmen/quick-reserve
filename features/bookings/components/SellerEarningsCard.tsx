...
interface SellerEarningsCardProps {
  booking: SellerEarningsBooking
}

const statusStyles: Record<SellerEarningsBooking['status'], string> = {
  confirmed: 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300',
  completed: 'bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-400',
}

/**
 * SellerEarningsCard: single paid booking row on the seller earnings page.
 *
 * Server component — mirrors the SellerBookingCard layout but is scoped to
 * `confirmed`/`completed` bookings only and emphasizes the amount earned with
 * a green "+" value instead of accept/decline controls.
 */
export function SellerEarningsCard({ booking }: SellerEarningsCardProps) {
  const { listing, slot, status } = booking

  return (
    <li
      className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900"
      data-testid="seller-earnings-card"
    >
